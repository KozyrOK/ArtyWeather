...
const theme = computed(() => state.settings?.user?.theme || user.value?.theme || localStorage.getItem('artyweather_theme') || 'light'); 
const t = computed(() => translations(locale.value)); 
const authenticated = computed(() => Boolean(user.value));

function applyClientPreferences(nextLocale, nextTheme) { 
    if (nextLocale) localStorage.setItem('artyweather_locale', nextLocale); 
    if (nextTheme) localStorage.setItem('artyweather_theme', nextTheme); 
}
...
async function initializeAuthenticatedUser() { 
    ...
    try { 
        ...
        applyClientPreferences(response.user?.locale, response.user?.theme); 
        await load(); 
    } catch (error) { 
        if (error?.status === 401) { 
            authToken.clear(); 
...
async function handleAuthenticated(authenticatedUser) { 
    ...
    applyClientPreferences(authenticatedUser.locale, authenticatedUser.theme); 
    await load(); 
}
...
async function handleLocaleChange(nextLocale) { 
    ...
    if (authenticated.value) await saveSettings({ locale: nextLocale, theme: theme.value }); 
}

async function handleThemeChange(nextTheme) { 
    applyClientPreferences(locale.value, nextTheme); 
    if (authenticated.value) await saveSettings({ locale: locale.value, theme: nextTheme }); 
}

async function handleSettingsChange(settings) { 
    applyClientPreferences(settings.locale || locale.value, settings.theme || theme.value); 
    await saveSettings(settings); 
    if (state.settings?.user) user.value = { ...user.value, ...state.settings.user }; 
}

watch(theme, (value) => { document.documentElement.dataset.theme = value === 'dark' ? 'dark' : 'light'; }, { immediate: true }); 
watch(locale, (value) => { document.documentElement.lang = value; }, { immediate: true }); 
onMounted(() => { window.addEventListener('hashchange', syncPageWithLocation);
window.addEventListener('popstate', syncPageWithLocation); 
initializeAuthenticatedUser(); }); onBeforeUnmount(() => { window.removeEventListener('hashchange', syncPageWithLocation);
